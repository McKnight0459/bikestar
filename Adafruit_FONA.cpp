...
boolean Adafruit_FONA::begin(Stream &port) {
  ...
  flushInput();
    ...
    // turn on Notifications for new SMS!
    sendCheckReply(F("AT+CNMI=2,1"), F("OK"));
    delay(100);
    if (! sendCheckReply(F("AT+CNMI=2,1"), F("OK"))) {
        return false;
    }
/*    // SET SMS ENGLISH!
    sendCheckReply(F("AT+CSCS=<GSM>"), F("OK"));
    delay(100);
    
    if (! sendCheckReply(F("AT+CSCS=<GSM>"), F("OK"))) {
        return false;
    }
*/
